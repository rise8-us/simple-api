...
from . import schema
from . import db
from .validation import verify, optional
...
def get_repositories():
    ...
    if (errors := verify(request_args, optional(schema.repository_schema), exclusive=True)):
...
def add_repository():
    ...
    if (errors := verify(request_body, schema.repository_schema)):
        return {"validation errors": errors}, 409
